...
function expressSvelte(mainOpts = {}) {
    ...
    async function svelteRender(name, renderOptions = {}) {
        ...
        try {
            ...
            let localProps = {};
            for(let key in this.app.locals){
                // if not a function, add to local props
                if(typeof this.app.locals[key] !== 'function'){
                    localProps[key] = this.app.locals[key];
                }
            }
            for(let key in this.locals){
                // if not a function, add to local props
                if(typeof this.locals[key] !== 'function'){
                    localProps[key] = this.locals[key];
                }
            }

            // Define request props based on argument options and defaults from middleware
            const props = renderOptions.props || {};
            const globalProps =  { ...renderOptions.globalProps, ...localProps };
